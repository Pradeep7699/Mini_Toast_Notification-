...
export default function ThirdComponent() {
  ...
  return (
    ...
    <div>
    {countries.length==0?(
<form onSubmit={handleCountdownSubmit} style={{ display: 'flex', flexDirection: 'column', marginTop: '50px', marginLeft: '100px' }}>
...
/>
<button style={{ width: '300px',borderRadius: '4px' ,height:"30px" }} type="submit">
  Start Timer
</button>
</form>
          ...
          <div key={toast.id}>
            {toast.remainingDuration > 0 && (
              <ToastNotification
                ...
                onMouseLeave={() => showToast(toast.remainingDuration, toast.remainingDuration)}
              />
            )}
            {toast.remainingDuration === 0 && (
              <ToastNotification
      ...
      <div>
        {currentCountries.map((country) => (
          <div style={{fontSize:"25px"}} key={country.country_id} >{country.country_name}</div>
        ))}
      ...
      <div style={{ textAlign: 'right', marginTop: '10px' }}>
        ...
        <button onClick={() => paginate(currentPage + 1)} disabled={indexOfLastCountry >= countries.length}>
          Next
        </button>
      </div>
    </div>
